...
- `OcrService` uses bundled Tesseract.js English data and a bundled native canvas renderer to recognize images and only PDF pages without extracted text. Large documents are processed sequentially with ten-page database checkpoints, allowing later runs to resume from preserved recognized pages without changing source files.
- `EducationService` validates and copies declared course manifests and Markdown/text lessons beneath `Content/Education`, while completion state remains in portable SQLite.
...
- `MapService` validates and copies user-selected PMTiles/MBTiles archives, provides rate-limited and locally cached user-triggered place search, invokes the pinned official Protomaps extractor only after the user confirms a center/radius/detail combination, verifies the resulting PMTiles archive, reads raster or vector MBTiles through SQLite, serves local tiles through range-aware internal protocols, and owns saved places plus GPX exchange.
