...
- `OcrService` uses bundled Tesseract.js English data and a bundled native canvas renderer to recognize images and blank PDF pages locally, reports per-page progress, and merges results into the existing document search index without changing source files.
