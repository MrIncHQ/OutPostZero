...
- `MapService` validates and copies user-selected PMTiles/MBTiles archives, can invoke the pinned official Protomaps extractor for a user-selected region from the current daily build, verifies the resulting PMTiles archive, reads raster or vector MBTiles through SQLite, serves local tiles through range-aware internal protocols, and owns saved places plus GPX exchange.
