...
export class NatureService {
  ...
  private catalog(): NatureCatalogEntry[] {
    const file = this.paths.resolve('Config/nature-catalog.json'); if (!fs.existsSync(file)) return [];
    try {
      const parsed = JSON.parse(fs.readFileSync(file, 'utf8')) as { entries?: unknown[] };
      return (parsed.entries ?? []).flatMap((raw) => {
        const item = raw as Partial<NatureCatalogEntry>;
        if (typeof item.id !== 'string' || !['pack', 'model'].includes(String(item.kind)) || typeof item.name !== 'string'
          ...
          || !Number.isSafeInteger(item.downloadBytes) || !Number.isSafeInteger(item.installedBytes) || typeof item.description !== 'string') return [];
        try { safeId(item.id); return [item as NatureCatalogEntry]; } catch { return []; }
      });
    } catch { return []; }
  }
  ...
  search(query: string, packIds?: string[], limit = 80): NatureSpeciesSummary[] {
    ...
    return this.queryPacks(packIds, (packId, db) => (db.prepare(`SELECT s.id, s.scientific_name, s.common_name, s.rank, s.category,
      ...
      FROM species_fts f JOIN species s ON s.id = f.species_id WHERE species_fts MATCH ? ORDER BY bm25(species_fts) LIMIT ?`).all(expression, limit) as Array<Record<string, unknown>>)
  ...
  async downloadContent(entryId: string): Promise<NatureOperationResult> {
    ...
    try {
      ...
      if (entry.kind === 'pack') this.importPack(partial); else { const target = this.paths.resolve(`AI/Nature/Models/${safeId(entry.id)}-${safeId(entry.version)}.onnx`); fs.copyFileSync(partial, `${target}.installing`); fs.renameSync(`${target}.installing`, target); }
