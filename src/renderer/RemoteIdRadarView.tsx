...
const EMPTY_STATE: RemoteIdState = { installed: false, enabled: false, connection: 'disconnected', serialLinesReceived: 0, ignoredLinesReceived: 0, observationsReceived: 0, serialLog: [], contacts: [] };
...
export function RemoteIdRadarView({ packages, onModules }: { packages: MapPackage[]; onModules?: (modules: ModuleSummary[]) => void }) {
  ...
  return <section className="radar-workspace">
    ...
    {serialActive && <div className={state.serialLinesReceived ? 'radar-activity active' : 'radar-activity waiting'}><div><small>SERIAL LINES</small><b>{state.serialLinesReceived}</b></div><div><small>AIRCRAFT MESSAGES</small><b>{state.observationsReceived}</b></div><div><small>DEBUG / OTHER</small><b>{state.ignoredLinesReceived}</b></div><div><small>LAST ACTIVITY</small><b>{state.lastSerialLineAt ? new Date(state.lastSerialLineAt).toLocaleTimeString() : 'NONE'}</b></div><p>{activityMessage}</p><details className="radar-serial-console"><summary>VIEW ESP32 SERIAL OUTPUT <span>{state.serialLog.length} LINE{state.serialLog.length === 1 ? '' : 'S'}</span></summary><div className="radar-console-heading"><span>CURRENT CONNECTION · NEWEST 250 LINES RETAINED</span><span>115200 BAUD</span></div><pre>{state.serialLog.length ? state.serialLog.map((entry, index) => <code className={`serial-${entry.kind}`} key={`${entry.receivedAt}-${index}`}><time>{new Date(entry.receivedAt).toLocaleTimeString()}</time><span>{entry.line}{entry.truncated ? ' … [DISPLAY TRUNCATED]' : ''}</span></code>) : <code className="serial-empty">Waiting for serial output…</code>}</pre></details></div>}
